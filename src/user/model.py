...
def create_refresh_token(user_id: int, username: str) -> RefreshToken:
    ...
    expires_at = datetime.now(UTC).replace(tzinfo=None) + timedelta(seconds=settings.refresh_token_expire_seconds)
...
def validate_refresh_token(token: str) -> RefreshToken | None:
    ...
    # Check expiration (compare as naive UTC datetimes)
